...
In order to use this binding, you will have to register as a [Nest Developer](https://developer.nest.com/) and [Register a new product](https://developer.nest.com/products/new).  Make sure to grant all the permissions you intend to use _(Note: Be sure, at minimum, to enable "Themostat","Away" and "Camera")_, and leave the **Redirect URI** field **blank** for PIN-based authorization. At this point, you will have your `nest:client_id` (**Product ID**) and `nest:client_secret` (**Product Secret**).
